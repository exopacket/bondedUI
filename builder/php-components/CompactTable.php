...
class CompactTable {
    ...
    private $vue_filename = "compact-table.vue";
    private $template;
    private $scripts = array();

    public function __construct($path) {

        $full_path = $path . $this->vue_filename;

        $doc = new DOMDocument('1.0', 'iso-8859-1');
        $doc->loadHTMLFile($full_path);
        $items = $doc->getElementsByTagName("template");
        if(sizeof($items) == 1) {
            $item = $items->item(0);
            $this->template = $item->C14N();
        }
        $scripts = $doc->getElementsByTagName("script");
        foreach($scripts as $script) {
            array_push($this->scripts, $script->C14N());
        }

        $this->full_content = file_get_contents($full_path);

    }

    public function getVueFile() {
